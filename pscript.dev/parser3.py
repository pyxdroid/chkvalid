...
class Parser3(Parser2):
    ...
    def function_isinstance(self, node):
        ...
        cmp_lower = cmp.lower()
        if cmp_lower == 'set':
            return [ob, 'instanceof', 'Set']
        elif cmp_lower == 'map':
            return [ob, 'instanceof', 'Map']
        if cmp_lower in BASIC_TYPES:
    ...
    def function_len(self, node):
        if len(node.arg_nodes) == 1:
            ...
            # not all objects have correct length
            return self.use_std_function('len', node.arg_nodes)
        else:
    ...
    def function_set(self, node):
        if len(node.arg_nodes) == 0:
            return 'new Set()'
        if len(node.arg_nodes) == 1:
            return self.use_std_function('set', node.arg_nodes)
        else:
            raise JSError('set() needs at least one argument')
...
def make_function(name, nargs, function_deps, method_deps):
    def function_X(self, node):
        ...
        if nargs != (-1,) and len(node.arg_nodes) not in nargs:
