...
class Parser2(Parser1):
    ...
    def parse_Assert(self, node):
        ...
        code.append(";}")
        self.use_std_class('AssertionError')
        return code
    ...
    def parse_ExceptHandler(self, node):
        ...
        #print ('parse_ExceptHandler', self._stack, node )
        err_type = unify(self.parse(node.type_node)) if node.type_node else ''
        self.vars.discard(err_type)
        if err_type:
            if err_type != 'Exception':
               code.append('if (%s instanceof Error && %s.name === (%s && %s.name ? %s.name: "%s")) {' %
                        (err_name, err_name, err_type, err_type, err_type, err_type))
            else:
               code.append('if (true) {')
        else:
            code.append('{')
    ...
    def parse_For(self, node):
        ...
        #print ('parse_For', self._stack, node )
        ...
        if isinstance(node.target_node, ast.Name):
            ...
            if sure_is_dict:
                ...
                if len(target) not in (1,2) or len(target) == 2 and sure_is_dict != 'items':
                    raise JSError('Iteration over a dict needs one iterator, '
                                  'or max 2 when using .items()')
            elif sure_is_range:
        ...
        if sure_is_range:  # Explicit iteration
            ...
            # JS uses  keys(), values() and entries()
            # Python   keys(), values() and items()
            if sure_is_dict == 'items':
                sure_is_dict = 'entries'  # in JS
            did_indent = False

            iterator = "(%s.%s)?%s.%s():Object.%s(%s)"%(iter, sure_is_dict, iter, sure_is_dict,sure_is_dict,iter)

            if len(target) == 1:
                if sure_is_dict == 'entries':
                   d_k = self.dummy('k')
                   d_v = self.dummy('v')
                   #code.append(self.lf("for (const [%s,%s] of Object.%s(%s)) {" %(d_k,d_v,sure_is_dict, iter)))
                   code.append(self.lf("for (const [%s,%s] of %s) {" %(d_k,d_v, iterator)))
                   self._indent += 1
                   did_indent = True
                   code.append(self.lf("%s=[%s,%s]" % (target[0],d_k, d_v)))
                else:
                   #code.append(self.lf("for (%s of Object.%s(%s)) {" %(target[0], sure_is_dict, iter)))
                   code.append(self.lf("for (%s of %s) {" %(target[0], iterator)))
            else:
                # always 2
                #code.append(self.lf("for ([%s,%s] of Object.%s(%s)) {" %(target[0], target[1], sure_is_dict, iter)))
                code.append(self.lf("for ([%s,%s] of %s) {" %(target[0], target[1], iterator)))
            ...
            if len(target) == 1:
                # use the standard JS iter protocol
                code.append(self.lf("for (%s of (%s[Symbol.iterator])?%s:Object.keys(%s)) {" %(target[0], iter,iter,iter)))
                self._indent += 1
            elif len(target) == 2:
                # ditto
                code.append(self.lf("for ([%s,%s] of %s) {" %(target[0], target[1], iter)))
                self._indent += 1

            else:
                # XXX do better
                #### raise JSError('not supported for-loop')
    ...
    def parse_FunctionDef(self, node, lambda_=False, asyn=False):
        ...
        #print ('parse_FunctionDef:', self._stack[-1], node)

        # check if function has 'self/this' as first param
        ...
        decos = []
        dummy_func = '' # temp funcname
        real_func = ''  # real funcname

        add_func_name = '' # add funcname to stack
        add_seen_func_name = ''
        ...
        if not lambda_:
            if node.decorator_nodes:
                #print('decos:', node.decorator_nodes)
                for _n in node.decorator_nodes:
                  if isinstance(_n, ast.Name):
                    if _n.name == 'staticmethod':
                       is_staticmethod = True
                       continue # ignore ?
                    elif _n.name == 'classmethod':
                       is_classmethod = True
                       continue # ignore ?
                    decos.append(_n.name)
                  else:
                    raise JSError('not known function decorator node:'+str(_n))
            if is_classmethod and is_staticmethod:
                raise JSError('staticmethod and classmethod not allowed together')

            if decos and (is_classmethod or is_staticmethod):
                # XXX check
                raise JSError('decorator not together with staticmethod or classmethod')

            prefixed2 = ''
            if not p_class_name:
                # not in class, normal function
                prefixed = node.name
                if not has_self:
                    func_name = 'flx_' + node.name
                add_func_name = node.name
                add_seen_func_name = add_func_name
            else:
                # method
                ##prefixed =self.with_prefix(node.name)
                prefixed = p_class_name + '.prototype.' + node.name
                #if is_staticmethod or is_classmethod:
                    #prefixed2 = self.with_prefix(node.name, noproto=True)
                    #prefixed2 += ' = '
                    #if is_classmethod:
                    #    # need it later to assign to first param
                    #    class_name = self.get_class_name()
                add_func_name = node.name
                add_seen_func_name = prefixed
            if decos:
                dummy_func = self.dummy('func')
                real_func = prefixed
                prefixed = dummy_func

            init_code.append(self.lf('%s%s = ' %(prefixed2, prefixed)))

        def add_function_init(is_generator):
            # will be called when is_generator is calculated
            init_code.append('%s%sfunction%s %s%s(' % ('(' if binder else '',
                                          'async ' if asyn else '',
        ...
        # perhaps args with default values must be renamed
        # because the name of default value is also in parameter name
        arg_renames = {}

        for arg in node.arg_nodes:  # ast.Arg nodes
            if is_classmethod and not varname_cls:
                # in a classmethod we must eat the first name
                varname_cls = arg.name
                continue

            name = self.NAME_MAP.get(arg.name, arg.name)
            if name != 'this':
                argnames.append(name) # this name is later known
                if arg.value_node is not None:
                   # default value
                   # print('arg.value_node', arg.value_node)
                   d = ''.join(self.parse(arg.value_node))
                   # if the default param is a name, it could
                   # clash the parameter name.
                   # Therefor a dummy name is used then.
                   if isinstance( arg.value_node, ast.Name):
                       arg_name = self.dummy('arg_'+name, True) # a 'tmp' dummy name
                       arg_renames[name] = arg_name
                       name = arg_name
                   name += ( '=' + d )
                # Add code and comma
                code.append(name)
        ...
        if is_classmethod:
            pre_code.append(self.lf('const %s = %s;'%(varname_cls, p_class_name)))
        ...
        if node.args_node:
            ...
            if not argnames:
                ...
                # always use a stub variable here, because of js overwriting of arguments
                args_var = self.dummy('args')
                vararg_code2 = '%s = %s.slice(%i);' % (name, args_var, len(argnames))
        ...
        for k,v in arg_renames.items():
           x = 'let %s = %s;' % (k, v)
           code.append(self.lf(x))
        #for arg in node.arg_nodes:
        #    if arg.value_node is not None:
        #        name = arg.name
        #        arg_name = arg_renames.get(name, name)
        #        d = ''.join(self.parse(arg.value_node))
        #        x = 'var %s = (%s === undefined) ? %s: %s;' % (name, arg_name, d, arg_name)
        #        code.append(self.lf(x))
        ...
        if lambda_:
          content_code = ['return ']
          content_code += self.parse(node.body_node)
          content_code.append(';')
        else:
          content_code = []
          ...
          if docstring and not node.body_nodes:
            ...
            for line in docstring.splitlines():
              content_code.append(self.lf(line))
          else:
            # Normal function
            if self._docstrings:
              for line in docstring.splitlines():
        ...
        self._indent -= 1
        if not lambda_:
            code.append(self.lf('}%s;' % binder))

            # this var must be bound to class as property too
            if p_class_name and add_func_name and add_seen_func_name:
              code += self.get_class_property_code(add_func_name, add_seen_func_name, ro=True);

            if dummy_func and decos:
                codechain = '('.join(decos)
                code.append(self.lf('%s = %s(%s%s;' % (real_func, codechain, dummy_func, ')'*len(decos) )))
            code.append('\n')

        add_function_init(is_generator)

        if add_func_name:
          self.vars.add(add_func_name)
        if add_seen_func_name:
          self._seen_func_names.add(add_seen_func_name)

        return init_code + pre_code + code
    ...
    def parse_ClassDef(self, node):
        #print ('parse_ClassDef:', self._stack[-1], node)
        ...
        if not base_class.replace('.', '_').isalnum():
            ...
            base_class = 'Object'
        ##else: mst:  ????
        ##    get_class_definition must be modified too ""
        ...
        #is_local = False  # XXX
        #if len(self._stack) >1 and self._stack[-1][0][:8] == 'function':
        #   is_local = True
        is_local = True   # probably ok

        for line in get_class_definition(node.name, base_class, docstring, is_local):
            code.append(self.lf(line))

        self.use_std_function('op_instantiate_chk', [])
        ...
        if is_local:
            self.vars.declare_var(node.name) # variable already declared in get_class_definition()
    ...
    def parse_Yield(self, node):
        ns = self._stack[-1]
        if not (ns[0][:8] == 'function'):
            raise JSError('yield must be inside a function.')
        if ns[0][-1] != '*':
            ns[0] += '*' # mark as generator
        #print(self._stack[-1])
        return 'yield %s' % ''.join(self.parse(node.value_node))
...
def get_class_definition(name, base='Object', docstring='', is_local=False):
    """ Get a list of lines that defines a class in JS.
    Used in the parser as well as by flexx.app.Component.
    """
    code = []

    code.append('%s%s = function () {' %('const ' if is_local else '',name))
    for line in docstring.splitlines():
        code.append('    // ' + line)

    if name[0].lower() == name[0]:
      # perhaps this classes will be called without new, because they are in lower case
      # e.g. 'datetime'
      code.append('    if (%sop_instantiate_chk(this)){' % stdlib.FUNCTION_PREFIX)
      code.append('      return new %s (... arguments);'%(name) )
      code.append('    }')
